...
class SiteScrubber {
  ...
  logDebugNaked(str: any) {
    if (this.o_debug) this.logNative(str);
  }
  ifElementExists(query: string, fn: Function = () => void 0) {
    return this.$(query) && fn(this.$(query));
  }
  addCustomCSSStyle(cssStr: string) {
    if (!cssStr) {
      return;
    }
  ...
  async waitUntilSelector(selector: string): Promise<HTMLElement> {
  ...
  async waitUntilGlobalVariable(...variableNames: Array<string>) {
    this.logDebug(
  ...
  removeElements(elements: Array<string> | string) {
    if (!elements) {
      return;
    }
    this.logDebug("Running removeElements");

    let elementsToRemove: Array<string> = [];
    if (typeof elements == "string" || elements instanceof String) {
      // add it to an array so we can use Array functions
      elementsToRemove.push(elements as string);
    }
    this._removeElementWatcherList = elementsToRemove;
    elementsToRemove.forEach((e: any) => {
      if (typeof e == "string" || e instanceof String) {
        // remove found elements
        this.$$(e).forEach((ele: HTMLElement) => {
  ...
  removeElementsByRegex({ query, regex }: removeElementsByRegexType) {
    if (!query) {
      return;
    }
    this.logDebug("Running removeElementsByRegex");
    this.$$(query).forEach((ele: HTMLElement) => {
  ...
  addInterval({ fn, interval, customID }: addIntervalType) {
    let error = false;
  ...
  addTimeout({ fn, timeout, customID }: addTimeoutType) {
    let error = false;
    if ("function" !== typeof fn) {
      ...
      "string" !== typeof customID ||
      !((customID as any) instanceof String)
    ) {
  ...
  addListener({ element, event, listener, options }: addListenerType) {
    let error = false;
  ...
  removeListener({ element, event }: removeListenerType) {
  ...
  async createGoogleRecaptcha(
    elementTarget: HTMLElement | string,
    site_key: string,
    position: insertLocation = "beforeend"
  ) {
  ...
  removeIFrames() {
    ...
    let i = 0;
    this.$$("iframe").forEach((iframe: HTMLIFrameElement) => {
  ...
  findParentElementByTagName(el: HTMLElement, tagName: string) {
  ...
  checkIfDownloadPage(
    arrayOfSelectors: Array<string> = [],
    arrayOfRegexTests: Array<RegExp> = []
  ) {
    if (
      ...
      (arrayOfRegexTests instanceof Array &&
        arrayOfRegexTests.some((regex: RegExp) =>
  ...
  isQueryString(query: string) {
    return (
      !this.isElement(query) &&
      (typeof query == "string" || (query as any) instanceof String)
    );
  }
  isEmptyObject(obj: object) {
    return JSON.stringify(obj) === "{}";
  }
  isEmptyArray(arr: Array<any>) {
  ...
  getDOMElement(request: HTMLElement | string) {
  ...
  addHoverAbility(
    ...
    [...element].forEach((e) => {
      if (typeof e == "string" || (e as any) instanceof String) {
        this.$$(e).forEach(addEvent);
      } else if ((e as any) instanceof HTMLElement) {
        addEvent(e);
      }
    });
  }
  addInfoBanner({ targetElement, where = "beforeend" }: addInfoBannerType) {
    if (targetElement instanceof HTMLElement) {
      // Already an HTMLElement
    } else if (
      typeof targetElement == "string" ||
      (targetElement as any) instanceof String
    ) {
      targetElement = this.$(targetElement) as HTMLElement | null;
    }
    if (!targetElement) {
      return;
    }
  ...
  destroyWindowFunctions(options: Array<string> = []) {
  ...
  interceptAddEventListeners(fn: Function) {
    this.log("Adding addEventListener hook");
    const _this = this;
    EventTarget.prototype.addEventListener = function (
      event: string,
      listener: EventListenerOrEventListenerObject | Function | null,
      bubbling: boolean | AddEventListenerOptions | undefined
    ): void {
      let allow = true;
      if (listener === null) {
        return;
  ...
  interceptXMLHttpRequest(fn: Function) {
  ...
  interceptPreventDefault(fn: Function) {
  ...
  interceptAppendChild(fn: Function) {
    const _this = this;
  ...
  createCountdown({ element, timer }: createCountdownType) {
  ...
  copyAttributesFromElement(sourceElement: HTMLElement, targetElement: HTMLElement) {
    if (
      sourceElement instanceof HTMLElement &&
      targetElement instanceof HTMLElement
    ) {
      [...sourceElement.attributes].forEach((attr: any) => {
...
const siteRules: siteRules = {
  ...
  chedrive: {
    ...
    removeByRegex: [
      {
        query: "div[class='txt']",
        regex:
          /what is chedrive|Why to Use Chedrive|Chedrive File Hosting|cloud computing/gi,
      },
      {
    ...
    createCountdown: undefined, // ".seconds" - but page doesnt check if you waited or not
    modifyButtons: [
      [
        "button#downloadbtn.downloadbtn",
        { customText: "Create Download Link" },
...
for (const site in siteRules) {
  ...
  if (
    currSiteRules.host.some((urlMatch: any) => {
