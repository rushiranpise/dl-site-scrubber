...
class SiteScrubber {
    o_debug = true;
    ssButtonWatchDog = true;
    window = window;
    document = window.document;
    logNative = window.console.log;
    console = window.console;
    openNative = window.open.bind(window);
    ...
    origAppendChild = HTMLElement.prototype.appendChild;
    ...
    addCustomCSSStyle(cssStr) {
        ...
        const targ = this.document.head || this.document.body || this.document.documentElement;
    ...
    async waitUntilGlobalVariable(...variableNames) {
        this.logDebug(`Waiting for global variable: window.${variableNames.join(".")}`);
        let curr = window;
    ...
    removeInterval(id) {
        ...
        if (interval) {
            ...
            this.logDebug(`removeInterval() - Failed to remove interval with ID: ${id}`);
        }
    }
    ...
    removeTimeout(id) {
        ...
        if (timeout) {
            ...
            this.logDebug(`removeTimeout() - Failed to remove timeout with ID: ${id}`);
        }
    }
    ...
    removeListener({ element, event }) {
        ...
        const removeObj = this._listeners.find((x) => x.element == el && x.listener == listener);
    ...
    async createGoogleRecaptcha(elementTarget, site_key, position = "beforeend") {
        ...
        this.waitUntilGlobalVariable("grecaptcha").then(() => {
            target.insertAdjacentHTML(position, `<div id="ss-recaptcha" data-sitekey="${site_key}" data-starttime="${+new Date()}"></div>`);
            // @ts-ignore
    ...
    checkIfDownloadPage(arrayOfSelectors = [], arrayOfRegexTests = []) {
        if ((arrayOfSelectors instanceof Array &&
            arrayOfSelectors.some((selector) => Boolean(this.document.querySelector(selector)))) ||
            (arrayOfRegexTests instanceof Array &&
                arrayOfRegexTests.some((regex) => regex?.test(this.document.documentElement.innerHTML)))) {
        ...
        this.logDebug(`checkIfDownloadPage() - ${arrayOfSelectors}, ${arrayOfRegexTests}`);
        return false;
    }
    isElement(element) {
    ...
    addInfoBanner({ targetElement, where = "beforeend" }) {
        ...
        this.logDebug(`addInfoBanner() - elementToAddTo: ${targetElement}, ${where}`);
    }
    ...
    interceptAppendChild(fn) {
        ...
        const customAppendChild = function (node) {
            ...
            if (allow) {
                ...
                _this.logDebug(`Intercepted attaching event listener: '${node.tagName}'`);
            }
        };
    ...
    modifyButton(button, { replaceWithForm = false, replaceWithTag, copyAttributesFromElement, customText, className, props, styles, attributes, eventHandlers, makeListener, requiresCaptcha, requiresTimer, addHoverAbility, moveTo = {
        target: undefined,
        position: undefined,
    ...
    applyRules() {
        if (!this.checkIfDownloadPage(this.currSiteRules?.downloadPageCheckBySelector, this.currSiteRules?.downloadPageCheckByRegex) &&
            // @ts-ignore
        ...
        this.currSiteRules?.removeByRegex?.forEach((removeByRegexOptions) => this.removeElementsByRegex(removeByRegexOptions));
        this.log("Removed elements");
        this.console.groupEnd();
        this.console.groupCollapsed("ss-addInfoBanner");
        this.currSiteRules?.addInfoBanner?.forEach((addInfoBannerOptions) => this.addInfoBanner(addInfoBannerOptions));
        this.console.groupEnd();
...
const siteRules = {
    dropapk: {
        ...
        customScript() {
            ...
            const isCaptchaBoxPresent = captchaTable.querySelector("b")?.textContent == "Enter code below:";
            if (isCaptchaBoxPresent) {
                const captchaCode = [
                    ...captchaTable.querySelectorAll("tr:nth-child(2) td:nth-child(1) > div span"),
                ]
                    .sort((x, y) => x.getAttribute("style").match(/padding-left:(\d+)/)?.[1] -
            ...
            // aesthetics
            this.ifElementExists("div[style*='direction:ltr']", (div) => (div.style.background = "#000"));
            this.$$(".col-6").forEach((div) => div.classList.replace("col-6", "col-12"));
            [...this.$$("div")]
    ...
    mixloads: {
        ...
        customScript() {
            ...
            this.ifElementExists("div.download_box img", () => {
                this.$("div.download_box").insertAdjacentHTML("afterbegin", '<div class="input-group mb-3"></div><div class="input-group-prepend text-center"></div><span class="input-group-text font-weight-bold">Captcha Code </span>');
                this.$("div.download_box span.input-group-text").appendChild(this.$("input.captcha_code"));
                this.$("input.captcha_code")?.classList.add("form-control");
                this.$("div.download_box").insertAdjacentElement("afterbegin", this.$("img"));
    ...
    uploadrar: {
        ...
        customScript() {
            ...
            const fileId = document.location.href.slice(document.location.href.lastIndexOf("/") + 1);
            ...
            [
                "div.in div.download-ready div.btm div.box-download a.btn.btn-download",
                {
                    fn() {
                        ...
                        const ddlURL = document.body.textContent.match(/return \'(http[s]?:\/\/(.*)?download(.*)?)\'/)?.[1] ?? null;
                        if (ddlURL) {
            ...
            if (this.$(".download-timer")) {
                ...
                if (table_header &&
                    ...
                    document
                        ...
                        ?.insertAdjacentHTML("afterend", `<p>siteScrubber is stopping because you've reached the download limit. (Use a VPN to get a new IP address)</p>`);
                    return;
                }
                sid = null;
                fetch(`https://rapidgator.net/download/AjaxStartTimer?fid=${fid}`, {
                    ...
                    document
                        .querySelector(".download-timer")
                        ?.insertAdjacentHTML("afterend", `<p>1st Request=${JSON.stringify(data)}</p>`);
                    sid = data.sid;
                });
                setTimeout(() => {
                    fetch(`https://rapidgator.net/download/AjaxGetDownloadLink?sid=${sid}`, {
                        ...
                        document
                            .querySelector(".download-timer")
                            ?.insertAdjacentHTML("afterend", `<p>2nd Request=${JSON.stringify(data)}</p>`);
                        document
                            .querySelector(".download-timer")
                            ?.insertAdjacentHTML("afterend", `<p>Ready to go to CAPTCHA page. Click the button to navigate to the next page.</p>`);
                        document
    ...
    katfile: {
        ...
        customScript() {
            ...
            this.$("form#btn_download")?.insertAdjacentHTML("afterbegin", `<input type="hidden" name="method_free" value="SLOW SPEED DOWNLOAD">`);
        },
    },
    "upload-4ever": {
    ...
    file4: {
        ...
        customScript() {
            ...
            this.waitUntilSelector(".div1").then((div) => (div.style.display = "none"));
    ...
    dailyuploads: {
        ...
        customScript() {
            this.waitUntilGlobalVariable("grecaptcha").then(() => {
                ...
                this.$("#downloadBtnClick")?.addEventListener("click", function () {
    ...
    usersdrive: {
        ...
        createCountdown: { element: ".seconds" },
        customScript() {
            this.$$(".row .col-md-4").forEach((e) => e.classList?.replace("col-md-4", "col-md-12"));
        },
    },
    indishare: {
        ...
        customScript() {
            ...
            if (window.fid) {
                ...
                this.sleep(1000).then(() => {
                    fetch(`/get_file.php?fid=${window.fid}&challenge=undefined&response=undefined&t=1`)
                        .then((res) => res.text())
                        .then((html) => html.match(/action="([^"]+)"\smethod/)?.[1])
                        .then((dl_link) => {
                        target.innerHTML = "";
                        target.insertAdjacentElement("afterbegin", this.makeSafeForm({ actionURL: dl_link }));
                    });
                });
            }
        },
        ...
        customScript() {
            ...
            if (captcha_box) {
                ...
                const captcha_code = [...captcha_box?.children]
                    .sort((x, y) => x.style?.paddingLeft.match(/(\d+)/g)[0] -
    ...
    douploads: {
        ...
        customScript() {
            ...
            this.window?.["setStyleSheet"]?.("https://douploads.net/doup1/assets/styles/dark.min.css");
            // Error Checks
            if (/proxy not allowed/gi.test(this.$("center div.alert.alert-danger")?.textContent)) {
    ...
    upfiles: {
        ...
        customScript() {
            ...
            this.waitUntilSelector("div#captchaDownload").then((captchaDownloadContainer) => {
                this.createGoogleRecaptcha(captchaDownloadContainer, "6LcsK9kaAAAAABe3I5PTS2zqmeKl3XueBrKNk3-Z");
            });
    ...
    centfile: {
        ...
        customScript() {
            this.$$(".col-md-4").forEach((e) => e.classList.replace("col-md-4", "col-md-12"));
            ...
            this.waitUntilSelector("div > span > a[href][onclick]").then((dl_btn) => {
                ...
                target?.insertAdjacentElement("beforeend", this.makeSafeForm({ actionURL: dl_btn.href, method: "GET" }));
    ...
    chedrive: {
        ...
        customScript() {
            this.$("div.col-xs-12.col-sm-12.col-md-4.col-lg-4")?.setAttribute("class", "col-12");
            this.$("div.col-xs-12.col-sm-12.col-md-4.col-lg-4")?.setAttribute("class", "col-12");
            ...
            if (this.$("#fileId")) {
                ...
                fetch("https://nitro.download/ajax/setCookie.php", {
                    ...
                    fetch("https://nitro.download/ajax/freeDownload.php", {
                        ...
                        .then((data) => {
                        if (data == "1") {
                            this.$("#beforeReCaptcha").innerHTML = `Wait <span class="seconds">120</span> seconds`;
    ...
    dropgalaxy: {
        ...
        customScript() {
            ...
            this.window?.["setStyleSheet"]?.("https://dropgalaxy.com/assets/styles/dark.min.css");
            this.$$(".col-md-4").forEach((ele) => ele.classList.replace("col-md-4", "col-md-12"));
            // enable right-click
                ...
                for (const mutation of mutationList) {
                    ...
                    for (const node of list) {
                        if (node?.tagName === "IFRAME" ||
                            ...
                            this.logDebug(`MutationObserver - Modded ${node?.tagName}:`, node);
                        }
                    }
                }
            };
            ...
            if (this.$("#xd")) {
                ...
                document.body.insertAdjacentHTML("beforeend", `<div style="display:none" class='ss-hide'>${divs}</div>`);
    ...
    beingupdated: {
        ...
        customScript() {
            const safeLinkURL = window.document.documentElement.outerHTML.match(/window.open\('(https:\/\/beingupdated.com\?safelink[^']+)'/i)?.[1];
            if (safeLinkURL) {
                [...document.body.children].forEach((element) => element.remove());
                document.body.className = "container";
                document.body.insertAdjacentHTML("afterbegin", `<a href="${safeLinkURL}" class="ss-animated-button ss-btn-ready ss-w-100">Continue to Next Step<span></span><span></span><span></span><span></span></button>`);
    ...
    gplinks: {
        ...
        customScript() {
            ...
            const redirectLink = window.document.documentElement.outerHTML.match(/redirectLink = '([^']+)'/)?.[1];
            if (redirectLink) {
                document
                    .querySelector(".ss-animated-button")
            ...
            if (form) {
                ...
                setTimeout(() => {
                    fetch("https://gplinks.co/links/go", {
                        ...
                        [...document.body.children].forEach((element) => element.remove());
                        document.body.className = "container";
                        document.body.insertAdjacentHTML("afterbegin", `<a href="${link}" class="ss-animated-button ss-btn-ready ss-w-100">${link}<span></span><span></span><span></span><span></span></button>`);
                        _this.addHoverAbility(document.querySelector(".ss-animated-button"), false);
                    });
...
for (const site in siteRules) {
    ...
    if (currSiteRules.host.some((urlMatch) => {
