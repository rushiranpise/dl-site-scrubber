// ==UserScript==
// @name         SiteScrubber - hexupload.net
// @namespace    SiteScrubber
// @version      1.0-20210727
...
/**
 * Used to monitor hCcaptcha and if the user completes
 * the tasks, then we submit the form automatically, if the wait
 * time has been exceeded as well
 * @param {HTMLElement} form <form> html tag
 * @param {int} timer seconds to wait before submitting
 * @returns undefined
 */
const hCaptchaListener = (form, timer = 0) => {
  if (form instanceof HTMLElement) {
    log("Form selected!");
  } else if (typeof form == "string" || form instanceof String) {
    // try to find form based on selector
  ...
  if (!form || !window.hcaptcha) {
    log("No hCaptcha found...");
    return;
  }
  // save current date
  const then = new Date();
...
const clean_site = () => {
  ...
  if (
    !checkIfDownloadPage(
      ["a.link.act-link.btn-free", "button#downloadbtn", "table.file_slot"],
      [
  ...
  GM_addStyle(`
    ...
    #container {background: #121212 !important;display: flex;flex-direction: column;justify-content: center;align-items: center;}
    .container {background: #121212 !important;display: flex;flex-direction: column;justify-content: center;align-items: center;}
  ...
  document.querySelector("input#btn_method_free")?.click();

  document.querySelector("#container > a.btn.btn-primary")?.click();
  ...
  removeElementsByRegex(
    "center",
    /All transactions are 100% safe and secure/gi
  );

  document.querySelectorAll("*").forEach((e) => e.setAttribute("style", ""));
