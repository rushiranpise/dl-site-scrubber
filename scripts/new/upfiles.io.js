// ==UserScript==
// @name         SiteScrubber - upfiles.io/upfiles.com
...
const clean_site = () => {
  ...
  waitUntilElementSelector_async("iframe[class][scrolling][sandbox]").then(
    (ele) => ele.remove()
  );

  // stop script from adding malicious redirects over top of google recaptcha
  appChild = document.body.appendChild;
  ...
  // global variables used by the ad system on this page
  window.a = undefined;
  window.e = undefined;

  // page 1
  document
    .querySelector("section.form-main.file-main form:not([id])")
    ?.submit();
  // page 2
  if (!document.querySelector("#captchaDownload")) {
  ...
  waitUntilElementSelector_async(
    ...
    if (/download link is almost ready/gi.test(document.body.innerText)) {
