// ==UserScript==
// @name         SiteScrubber - usersdrive.com
...
const clean_site = () => {
  ...
  GM_addStyle(`
    ...
    body {background: #121212 !important; color: #dfdfdf !important;padding:0 !important;}
    .container-fluid main {background: #121212 !important; padding:0 !important;}
    .down {display: flex !important;flex-direction: column !important;align-items: center !important;}
    `);

  // Remove crap
  removeElements(["nav", "center", ".col-md", ".socialmedia", ".pro"]);

  document.querySelector(".row .col-md-4")?.classList?.replace("col-md-4", "col-md-12");

  googleRecaptchaListener(document.forms.F1, 17);

  waitUntilElementSelector_async(".down a").then((res) => {
    if (/This direct link will be available/gi.test(document.body.innerText)) {
