// ==UserScript==
// @name         SiteScrubber - drop.download
...
const clean_site = () => {
  ...
  removeElementsByRegex(".download_method", /fast download/gi);

  const captcha_box = document.querySelector(".download_box div");
  if (captcha_box) {
      const captcha_code = [...captcha_box?.children].sort((x,y) => x.getAttribute("style").match(/padding-left:(\d+)/)?.[1] - y.getAttribute("style").match(/padding-left:(\d+)/)?.[1]).map(e => e.textContent).join("");
  ...
  waitUntilElementSelector_async("div.download_box a").then((res) => {
    if (/download now/gi.test(document.body.innerText)) {
